...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, nb_classes=101, channel=20):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0])            #making some custom layer
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2) #making some custom layer
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2) #making some custom layer
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2) #making some custom layer
        self.avgpool = nn.AvgPool2d(7)
        self.fc_custom = nn.Linear(512 * block.expansion, nb_classes)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                m.weight.data.normal_(0, math.sqrt(2. / n)) #weight normalization
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(planes * block.expansion), #normalizing our data
            )
...
def resnet101(pretrained=False, channel= 20, **kwargs):
    ...
    # print("model in resnet101: ", model)
    if pretrained:
